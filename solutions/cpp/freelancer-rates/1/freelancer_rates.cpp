...
double apply_discount(double before_discount, double discount) {
    ...
    after_discount = before_discount * ((100 - discount) / 100);
...
int monthly_rate(double hourly_rate, double discount) {
    ...
    double daily_salary;
    double result;
    daily_salary = hourly_rate * 8 * ((100 - discount) / 100);
    result = daily_salary * 22;
    // discount.
    return ceil(result);
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    double daily_salary = hourly_rate * 8 * ((100 - discount) / 100);
    return floor(budget / daily_salary);
}
